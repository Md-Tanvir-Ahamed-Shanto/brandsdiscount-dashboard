...
const useBulkUpdateMutation = () => {
    ...
    const updateBulk = async (data) => {
        ...
        setError(null);
        try {
            const response = await fetch(`${BASE_API_URL}/products/update-quantities`, {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || `Failed to update products: ${response.statusText}`);
            }
...
const BarCodeProductList = ({ scannedSkusFromScanner = [] }) => {
    ...
    const { searchResults, loadingSearch, errorSearch } = useProductSearch(searchTerm);
    ...
    const [updateBulk, { isLoading: isBulkUpdating, error: bulkUpdateError }] = useBulkUpdateMutation();
    ...
    const addOrUpdateProductInList = useCallback(async (sku, initialQuantity = 1) => {
        ...
        if (existingProduct) {
            ...
            toast.info(`Quantity for ${trimmedSku} updated to ${ (quantities[trimmedSku] || 0) + initialQuantity}.`);
            playCheckSound();
            return; 
        }
        ...
        playCheckSound();
    }, [productsInList, quantities, playCheckSound]); // Dependencies for useCallback

    useEffect(() => {
        if (scannedSkusFromScanner.length === 0) return;

        const latestScannedSku = scannedSkusFromScanner[scannedSkusFromScanner.length - 1];
    ...
    const handleManualSkuInputKeyDown = useCallback((e) => {
        if (e.key === 'Enter') {
    ...
    const confirmAllProducts = async () => {
        ...
        console.log("Confirming products for bulk update:", { data: productsDataForAPI });
        ...
        try {
            ...
            if (response?.success) {
                toast.success(`${productsDataForAPI.length} product(s) checked out successfully!`);
            } else if (!response?.success && response.results?.length > 0) {
                const successfulUpdates = response.results.filter(r => r.status === 'success').length;
                const failedUpdates = response.results.filter(r => r.status === 'failed').length;

                if (successfulUpdates > 0 && failedUpdates === 0) {
                    toast.success(`${successfulUpdates} product(s) updated successfully!`);
                } else if (successfulUpdates > 0 && failedUpdates > 0) {
                    toast.warn(`${successfulUpdates} products updated, but ${failedUpdates} failed. Check console for details.`);
                    response.results.filter(r => r.status === 'failed').forEach(f => {
                 ...
                 toast.error(response.message || "Bulk update failed with an unknown error.");
            }
    ...
    return (
        <div className="container mx-auto p-4 bg-gray-800 rounded-lg shadow-lg">
            ...
            {bulkUpdateError && (
                <p className="text-red-400 text-sm mb-4">Bulk update error: {bulkUpdateError.message}</p>
            )}

            {/* Manual SKU Direct Add Input Section */}
            <div className="mb-6 bg-gray-700 p-4 rounded-md shadow-inner">
                <h2 className="text-xl font-semibold text-white mb-3">Direct SKU Input (e.g., manual type or hardware scanner without active field)</h2>
                ...
                <p className="text-xs text-gray-400 mt-2">Use this for manual entry or if your barcode scanner is configured to directly type into an active input field.</p>
            </div>

            {/* Product Search Section */}
            <div className="bg-gray-700 p-4 rounded-md mb-6 shadow-inner">
                <h2 className="text-xl font-semibold text-white mb-3">Search & Add Product from Database</h2>
                ...
                {errorSearch && <p className="text-red-400 text-sm mb-2">Error searching: {errorSearch.message}</p>}

                {searchTerm.trim() && !loadingSearch && searchResults.length === 0 && !errorSearch && (
                    <p className="text-gray-400 text-sm mb-2">No products found for "{searchTerm}".</p>
                )}
                        ...
                        {searchResults.map(product => (
                            ...
                            >
                                <div className="flex-grow">
                                    ...
                                    <p className="text-sm text-gray-300">SKU: {product.sku} | Price: ${product.salePrice?.toFixed(2)}</p>
                                </div>
                                <button
            ...
            {productsInList.length === 0 ? (
                <div className="text-center p-8 border border-gray-600 rounded-md bg-gray-700 text-gray-300">
                    ...
                    <p className="text-sm mt-2">Scan a barcode, directly add an SKU, or search for products above.</p>
                </div>
            ) : (
                    ...
                    <table className="w-full border-collapse">
                        <thead>
                            <tr className="bg-gray-600">
                                <th className="border border-gray-500 px-4 py-2 text-left text-white text-sm">SKU</th>
                                <th className="border border-gray-500 px-4 py-2 text-left text-white text-sm">Product Name</th>
                                <th className="border border-gray-500 px-4 py-2 text-left text-white text-sm">Location</th>
                                <th className="border border-gray-500 px-4 py-2 text-left text-white text-sm">Brand</th>
                                <th className="border border-gray-500 px-4 py-2 text-left text-white text-sm">Price</th>
                                <th className="border border-gray-500 px-4 py-2 text-left text-white text-sm">Stock</th>
                                <th className="border border-gray-500 px-4 py-2 text-center text-white text-sm">Quantity</th>
                                <th className="border border-gray-500 px-4 py-2 text-center text-white text-sm">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {productsInList.map(product => (
                                <tr key={product?.sku} className="bg-gray-800 hover:bg-gray-700 transition-colors">
                                    <td className="border border-gray-700 px-4 py-2 font-medium text-white text-sm">{product?.sku}</td>
                                    <td className="border border-gray-700 px-4 py-2 text-gray-200 text-sm">{product?.title}</td>
                                    <td className="border border-gray-700 px-4 py-2 text-gray-200 text-sm">{product?.itemLocation}</td>
                                    <td className="border border-gray-700 px-4 py-2 text-gray-200 text-sm">{product?.brandName}</td>
                                    <td className="border border-gray-700 px-4 py-2 text-green-400 font-bold text-sm">${product?.salePrice?.toFixed(2)}</td>
                                    <td className="border border-gray-700 px-4 py-2 text-gray-200 text-sm">{product?.stockQuantity}</td>
