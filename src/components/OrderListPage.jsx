...
const Modal = ({ isOpen, onClose, title, children, size = 'md' }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className={`bg-gray-800 rounded-lg shadow-xl p-6 ${sizeClasses[size]} w-full mx-4`}>
...
const OrderListPage = () => {
  const { isAuthenticated, loading: authLoading, hasPermission, user: currentUser } = useContext(AuthContext); // Destructure currentUser
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || `Failed to update status for order ${orderId}.`);
  ...
  return (
    <div className="p-4 md:p-6 bg-gray-900 min-h-screen text-white">
      <div className="mb-6 flex flex-col sm:flex-row justify-between items-center">
        <h3 className="text-xl font-semibold text-white">All Orders ({totalItems})</h3>
      ...
      <div className="mb-6 flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0 md:space-x-4">
        <form onSubmit={handleSearchSubmit} className="flex flex-grow w-full md:w-auto">
          <input
            type="text"
          ...
          <table className="w-full text-sm text-left text-gray-400">
            ...
            <tbody>
              {orders.map(order => (
                <tr
                  key={order.id}
                  className={`border-b border-gray-700 hover:bg-gray-700/50 ${selectedOrders.has(order.id) ? 'bg-gray-700' : 'bg-gray-800'}`}
                >
                  ...
                  <td className="px-6 py-4 flex space-x-2">
                    <button onClick={() => handleViewOrder(order)} className="text-blue-400 hover:text-blue-300" title="View Order">
                      <Eye size={18} />
                    </button>
                    {canSendEmail && ( // Render email button only if user has permission
                      <button onClick={() => handleOpenEmailModal(order)} className="text-indigo-400 hover:text-indigo-300" title="Email Customer">
                        <Mail size={18} />
                      </button>
                    )}
                    {canDeleteOrder && (
        ...
        <div className="flex space-x-2">
          ...
          {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
            <button
              ...
              className={`px-4 py-2 rounded-md ${currentPage === page ? 'bg-indigo-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
      ...
      <Modal isOpen={isOrderModalOpen} onClose={() => setIsOrderModalOpen(false)} title={`Order Details: ${selectedOrder?.id}`} size="xl">
        {selectedOrder && (
          <div className="text-gray-300 space-y-3">
            ...
            <p><strong className="text-gray-100">Transaction ID:</strong> {selectedOrder.transaction?.transactionId || 'N/A'}</p>
            ...
            <div>
              ...
              <ul className="list-disc list-inside mt-1 space-y-1">
                {selectedOrder.orderDetails?.map((item) => (
                  <li key={item.id}>{item.quantity}x {item.productName} (@ ${item.price.toFixed(2)} each) - SKU: {item.sku}</li>
                ))}
              </ul>
            </div>
          </div>
      ...
      <Modal isOpen={isEmailModalOpen} onClose={() => setIsEmailModalOpen(false)} title={`Email Customer for Order ${selectedOrder?.id}`} size="xl">
        <div className="space-y-4 text-gray-300">
          ...
          <div className="flex justify-end">
            <button onClick={sendEmailToCustomer} className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded">
              Send Email
            </button>
          </div>
        </div>
