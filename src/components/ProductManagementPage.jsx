...
import { Search, PlusCircle, Filter, Edit, Check, EyeOff, Archive, DollarSign, Edit3, CheckCircle, ChevronLeft, ChevronRight } from 'lucide-react';
...
export const SIZE_TYPES = ['XXS (00)', 'XS (0-2)', 'S (4-6)', 'M (8-10)', 'L (12-14)', 'XXL+ (20+)', 'O/S', 'Other'];
...
const getStatusColor = (status) => {
  switch (status) {
    ...
    case 'Hidden': return 'bg-orange-600 text-white';
    case 'Draft': return 'bg-yellow-600 text-black';
    case 'Archived': return 'bg-red-600 text-white'; // 'Archived' is not in new PRODUCT_STATUSES but kept for robustness
...
const AddEditProductModal = ({ isOpen, onClose, productData, onSave, categories, brands, sizeTypes }) => {
    ...
    if (isOpen) {
      if (productData) {
        ...
        setFormData({
          ...
          toggleFirstDeal: productData.toggleFirstDeal || false,
  ...
  const validateForm = () => {
    ...
    if (!formData.stockQuantity || isNaN(Number(formData.stockQuantity))) errors.stockQuantity = 'Quantity must be a number.';
    if (!formData.regularPrice || isNaN(Number(formData.regularPrice))) errors.regularPrice = 'Regular price must be a number.';
    if (formData.salePrice && isNaN(Number(formData.salePrice))) errors.salePrice = 'Sale price must be a number.';
    if (formData.platFormPrice && isNaN(Number(formData.platFormPrice))) errors.platFormPrice = 'Platform price must be a number.';
    if (formData.discountPercent && isNaN(Number(formData.discountPercent))) errors.discountPercent = 'Discount percent must be a number.';
  ...
  const handleSubmit = (e) => {
    ...
    const dataToSave = {
      ...formData,
      regularPrice: parseFloat(formData.regularPrice) || null,
      salePrice: parseFloat(formData.salePrice) || null,
      platFormPrice: parseFloat(formData.platFormPrice) || null,
      discountPercent: parseFloat(formData.discountPercent) || null,
      stockQuantity: parseInt(formData.stockQuantity, 10) || null,
      // Pass updatedById from current user or context
      updatedById: MOCK_CURRENT_USER.id,
    };
    onSave(dataToSave);
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 rounded-xl shadow-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto p-6">
        <div className="flex justify-between items-center border-b border-gray-700 pb-4 mb-4">
          <h2 className="text-2xl font-bold text-white">{productData ? 'Edit Product' : 'Add New Product'}</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-white">
            <PlusCircle size={24} className="rotate-45" /> {/* Use plus for close, rotated */}
          </button>
        </div>
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* General Information */}
          <div className="col-span-1 md:col-span-2 text-white font-semibold mb-2">General Information</div>
          <div className="flex flex-col">
            <label htmlFor="title" className="text-gray-300 text-sm mb-1">Title <span className="text-red-500">*</span></label>
            <input type="text" id="title" name="title" value={formData.title} onChange={handleChange}
              className={`bg-gray-700 text-white rounded-lg p-2 focus:ring-indigo-500 focus:border-indigo-500 ${validationErrors.title ? 'border-red-500' : ''}`} />
            {validationErrors.title && <span className="text-red-500 text-xs mt-1">{validationErrors.title}</span>}
          </div>
          <div className="flex flex-col">
            <label htmlFor="sku" className="text-gray-300 text-sm mb-1">SKU <span className="text-red-500">*</span></label>
            <input type="text" id="sku" name="sku" value={formData.sku} onChange={handleChange}
              className={`bg-gray-700 text-white rounded-lg p-2 focus:ring-indigo-500 focus:border-indigo-500 ${validationErrors.sku ? 'border-red-500' : ''}`} />
            {validationErrors.sku && <span className="text-red-500 text-xs mt-1">{validationErrors.sku}</span>}
          </div>
          <div className="flex flex-col">
            <label htmlFor="brandName" className="text-gray-300 text-sm mb-1">Brand Name</label>
            <select id="brandName" name="brandName" value={formData.brandName} onChange={handleChange}
          ...
          <div className="flex flex-col">
            <label htmlFor="color" className="text-gray-300 text-sm mb-1">Color</label>
            <input type="text" id="color" name="color" value={formData.color} onChange={handleChange}
              className="bg-gray-700 text-white rounded-lg p-2 focus:ring-indigo-500 focus:border-indigo-500" />
          </div>
          <div className="flex flex-col">
            <label htmlFor="itemLocation" className="text-gray-300 text-sm mb-1">Item Location</label>
            <input type="text" id="itemLocation" name="itemLocation" value={formData.itemLocation} onChange={handleChange}
              className="bg-gray-700 text-white rounded-lg p-2 focus:ring-indigo-500 focus:border-indigo-500" />
          </div>
          <div className="flex flex-col">
            <label htmlFor="condition" className="text-gray-300 text-sm mb-1">Condition</label>
          ...
          <div className="col-span-1 md:col-span-2 text-white font-semibold mt-4 mb-2">Category & Size</div>
          <div className="flex flex-col">
            <label htmlFor="category" className="text-gray-300 text-sm mb-1">Category</label>
            <select id="category" name="category"
              value={categories.find(cat => cat.categoryId === formData.categoryId && cat.subCategoryId === formData.subCategoryId && cat.parentCategoryId === formData.parentCategoryId)?.name || ''}
          ...
          {/* Note: sizeId and sizeType are from the schema. A more robust solution might fetch available sizes based on category/type */}
          <div className="flex flex-col">
            <label htmlFor="sizeId" className="text-gray-300 text-sm mb-1">Size ID (Placeholder)</label>
            <input type="text" id="sizeId" name="sizeId" value={formData.sizeId} onChange={handleChange}
              className="bg-gray-700 text-white rounded-lg p-2 focus:ring-indigo-500 focus:border-indigo-500" />
          </div>
          <div className="flex flex-col">
            <label htmlFor="sizeType" className="text-gray-300 text-sm mb-1">Size Type</label>
          ...
          <div className="flex flex-col">
            <label htmlFor="postName" className="text-gray-300 text-sm mb-1">Post Name</label>
            <input type="text" id="postName" name="postName" value={formData.postName} onChange={handleChange}
              className="bg-gray-700 text-white rounded-lg p-2 focus:ring-indigo-500 focus:border-indigo-500" />
          </div>

          {/* Pricing & Inventory */}
          <div className="col-span-1 md:col-span-2 text-white font-semibold mt-4 mb-2">Pricing & Inventory</div>
          <div className="flex flex-col">
            <label htmlFor="regularPrice" className="text-gray-300 text-sm mb-1">Regular Price ($) <span className="text-red-500">*</span></label>
            <input type="number" step="0.01" id="regularPrice" name="regularPrice" value={formData.regularPrice} onChange={handleChange}
              className={`bg-gray-700 text-white rounded-lg p-2 focus:ring-indigo-500 focus:border-indigo-500 ${validationErrors.regularPrice ? 'border-red-500' : ''}`} />
            {validationErrors.regularPrice && <span className="text-red-500 text-xs mt-1">{validationErrors.regularPrice}</span>}
          </div>
          <div className="flex flex-col">
            <label htmlFor="salePrice" className="text-gray-300 text-sm mb-1">Sale Price ($)</label>
            <input type="number" step="0.01" id="salePrice" name="salePrice" value={formData.salePrice} onChange={handleChange}
              className={`bg-gray-700 text-white rounded-lg p-2 focus:ring-indigo-500 focus:border-indigo-500 ${validationErrors.salePrice ? 'border-red-500' : ''}`} />
            {validationErrors.salePrice && <span className="text-red-500 text-xs mt-1">{validationErrors.salePrice}</span>}
          </div>
          <div className="flex flex-col">
            <label htmlFor="platFormPrice" className="text-gray-300 text-sm mb-1">Platform Price ($)</label>
            <input type="number" step="0.01" id="platFormPrice" name="platFormPrice" value={formData.platFormPrice} onChange={handleChange}
              className={`bg-gray-700 text-white rounded-lg p-2 focus:ring-indigo-500 focus:border-indigo-500 ${validationErrors.platFormPrice ? 'border-red-500' : ''}`} />
            {validationErrors.platFormPrice && <span className="text-red-500 text-xs mt-1">{validationErrors.platFormPrice}</span>}
          </div>
          <div className="flex flex-col">
            <label htmlFor="discountPercent" className="text-gray-300 text-sm mb-1">Discount Percent (%)</label>
            <input type="number" step="0.01" id="discountPercent" name="discountPercent" value={formData.discountPercent} onChange={handleChange}
              className={`bg-gray-700 text-white rounded-lg p-2 focus:ring-indigo-500 focus:border-indigo-500 ${validationErrors.discountPercent ? 'border-red-500' : ''}`} />
            {validationErrors.discountPercent && <span className="text-red-500 text-xs mt-1">{validationErrors.discountPercent}</span>}
          </div>
          <div className="flex flex-col">
            <label htmlFor="stockQuantity" className="text-gray-300 text-sm mb-1">Stock Quantity <span className="text-red-500">*</span></label>
            <input type="number" id="stockQuantity" name="stockQuantity" value={formData.stockQuantity} onChange={handleChange}
              className={`bg-gray-700 text-white rounded-lg p-2 focus:ring-indigo-500 focus:border-indigo-500 ${validationErrors.stockQuantity ? 'border-red-500' : ''}`} />
            {validationErrors.stockQuantity && <span className="text-red-500 text-xs mt-1">{validationErrors.stockQuantity}</span>}
          </div>
          <div className="flex items-center gap-2 mt-4">
            <input type="checkbox" id="toggleFirstDeal" name="toggleFirstDeal" checked={formData.toggleFirstDeal} onChange={handleChange}
              className="form-checkbox h-4 w-4 text-indigo-600 bg-gray-600 border-gray-500 rounded focus:ring-indigo-500" />
            <label htmlFor="toggleFirstDeal" className="text-gray-300 text-sm">Toggle 10% discount for first deal</label>
          </div>

          {/* Platforms & Status */}
          <div className="col-span-1 md:col-span-2 text-white font-semibold mt-4 mb-2">Platforms & Status</div>
          <div className="flex flex-col">
            <label htmlFor="ebayId" className="text-gray-300 text-sm mb-1">eBay ID</label>
            <input type="text" id="ebayId" name="ebayId" value={formData.ebayId} onChange={handleChange}
              className="bg-gray-700 text-white rounded-lg p-2 focus:ring-indigo-500 focus:border-indigo-500" />
          </div>
          <div className="flex flex-col">
            <label htmlFor="wallmartId" className="text-gray-300 text-sm mb-1">Walmart ID</label>
            <input type="text" id="wallmartId" name="wallmartId" value={formData.wallmartId} onChange={handleChange}
              className="bg-gray-700 text-white rounded-lg p-2 focus:ring-indigo-500 focus:border-indigo-500" />
          </div>
          <div className="flex flex-col">
            <label htmlFor="sheinId" className="text-gray-300 text-sm mb-1">Shein ID</label>
            <input type="text" id="sheinId" name="sheinId" value={formData.sheinId} onChange={handleChange}
              className="bg-gray-700 text-white rounded-lg p-2 focus:ring-indigo-500 focus:border-indigo-500" />
          </div>
          <div className="flex flex-col">
            <label htmlFor="woocommerceId" className="text-gray-300 text-sm mb-1">WooCommerce ID</label>
            <input type="text" id="woocommerceId" name="woocommerceId" value={formData.woocommerceId} onChange={handleChange}
              className="bg-gray-700 text-white rounded-lg p-2 focus:ring-indigo-500 focus:border-indigo-500" />
          </div>
          <div className="flex flex-col">
            <label htmlFor="status" className="text-gray-300 text-sm mb-1">Status</label>
          ...
          {/* Images */}
          <div className="col-span-1 md:col-span-2 text-white font-semibold mt-4 mb-2">Images</div>
          <div className="col-span-1 md:col-span-2 flex flex-col gap-2">
            <div className="flex gap-2">
              <input type="text" placeholder="Add image URL" value={newImageUrl} onChange={(e) => setNewImageUrl(e.target.value)}
                className="flex-grow bg-gray-700 text-white rounded-lg p-2 focus:ring-indigo-500 focus:border-indigo-500" />
              <button type="button" onClick={handleAddImage} className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg">Add</button>
            </div>
            <div className="flex flex-wrap gap-2 mt-2">
              {formData.images.map((img, index) => (
                <div key={index} className="relative w-20 h-20 group">
                  <img src={img} alt={`Product image ${index}`} className="w-full h-full object-cover rounded-md" />
          ...
          {/* Description */}
          <div className="col-span-1 md:col-span-2 text-white font-semibold mt-4 mb-2">Description</div>
          <div className="col-span-1 md:col-span-2">
            <label htmlFor="description" className="sr-only">Description</label>
            <textarea id="description" name="description" value={formData.description} onChange={handleChange} rows="4"
              className="bg-gray-700 text-white rounded-lg p-2 focus:ring-indigo-500 focus:border-indigo-500 w-full"></textarea>
          </div>

          <div className="col-span-1 md:col-span-2 flex justify-end gap-3 mt-6">
            <button type="button" onClick={onClose} className="bg-gray-600 hover:bg-gray-500 text-white font-semibold px-5 py-2 rounded-lg">Cancel</button>
            <button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-5 py-2 rounded-lg">
...
const ProductManagementPage = () => {
  ...
  const canEditProducts = PERMISSIONS[currentUser.role]?.includes('editProduct');
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      if (filters.itemLocationFilter) queryParams.append('itemLocationFilter', filters.itemLocationFilter);

      const response = await fetch(`${BASE_API_URL}/products?${queryParams.toString()}`);
      if (!response.ok) {
  ...
  const saveQuantityChange = async (productId) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${BASE_API_URL}/products/${productId}/stock-quantity`, {
        method: 'PATCH',
  ...
  const handleQuickAction = async (productId, actionType) => {
    ...
    try {
      if (actionType === 'toggleOffer') {
        const response = await fetch(`${BASE_API_URL}/products/${productId}/toggle-offer`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
        });
        ...
        const response = await fetch(`${BASE_API_URL}/products/${productId}/status`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleBulkAction = async (actionType, value = null) => {
    ...
    try {
      ...
      if (actionType === 'updateInventoryBulk') {
        ...
        console.warn("Bulk inventory update requires specific quantity input for each selected product. This is a placeholder.");
        alert("Bulk inventory update selected. In a real app, you'd input quantities here.");
        setLoading(false);
        return;
      } else if (value !== null) {
        payload.value = value;
  ...
  const handleSaveProduct = async (productFormData) => {
    ...
    try {
      ...
      const url = currentEditingProductId ? `${BASE_API_URL}/products/${currentEditingProductId}` : `${BASE_API_URL}/products`;
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || `Failed to ${currentEditingProductId ? 'update' : 'add'} product`);
      }
  ...
  const productToEdit = currentEditingProductId ? products.find(p => p.id === currentEditingProductId) : null;
    ...
    <div className="min-h-screen bg-gray-900 text-white font-sans antialiased">
      <div className="p-4 md:p-6">
        ...
        <div className="mb-6 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div>
            <h3 className="text-xl font-semibold text-white">Products ({totalItems} total)</h3>
          </div>
        ...
        <div className="mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 p-4 bg-gray-800 rounded-lg shadow">
          ...
          >
            ...
            {ALL_CATEGORIES_FLAT.map(cat => <option key={cat.id} value={cat.name}>{cat.name}</option>)}
          </select>
          <select
            value={filters.brand}
          ...
          >
            ...
            {MOCK_BRANDS.map(brand => <option key={brand} value={brand}>{brand}</option>)}
          </select>
          <select
                  ...
                  <tr key={product.id} className={`border-b border-gray-700 hover:bg-gray-700/50 ${selectedProducts.has(product.id) ? 'bg-gray-700' : 'bg-gray-800'}`}>
                    ...
                    <td className="px-4 py-3 font-medium text-white whitespace-nowrap flex items-center">
                      <ImageWithFallback
                        ...
                        fallbackSrc={`https://placehold.co/40x40/777/FFF?text=${product.brandName ? product.brandName.substring(0, 1) : 'P'}`}
                        alt={product.title}
                        className="w-10 h-10 rounded-md object-cover mr-3"
                      />
                      <div>
                        <span className="truncate max-w-xs block" title={product.title}>{product.title}</span>
                        <span className="text-xs text-gray-500">
                          {product.category?.name} {/* Display category name if available */}
                          {product.variants && product.variants.length > 0 && <span className="text-blue-400">(Has Variants)</span>}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-center">
                      {typeof editingQuantity[product.id] !== 'undefined' && canEditProducts && !product.variants ? (
                        <div className="flex items-center justify-center">
                          ...
                          />
                          <button onClick={() => saveQuantityChange(product.id)} className="ml-1 text-green-400 hover:text-green-300">
                            <CheckCircle size={18} />
                          </button>
                        </div>
                      ) : (
                        <span
                          onClick={() => canEditProducts && !product.variants && setEditingQuantity({ ...editingQuantity, [product.id]: product.quantity })}
                          className={`${canEditProducts && !product.variants ? 'cursor-pointer hover:text-white' : ''} ${product.quantity === 0 ? 'text-red-400 font-semibold' : ''}`}
                        >
                          {product.quantity}
                        </span>
                      )}
                    ...
                    <td className="px-4 py-3">
                      {(canEditProducts || (currentUser.role === 'WarehouseUploader' && product.status === 'Draft')) && (
                        <div className="flex space-x-1 items-center">
                          <button onClick={() => handleEditClick(product.id)} className="text-blue-400 hover:text-blue-300 p-1" title="Edit Product">
                            <Edit3 size={16} />
                          </button>
        ...
        {paginatedProducts.length === 0 && !loading && !error && <p className="text-center text-gray-500 mt-8">No products found matching your criteria.</p>}
        <Pagination
